...
static const struct reg_default wm8960_reg_defaults[] = {
	{ 0x0, 0x00a7 },  { 0x1, 0x00a7 },  { 0x2, 0x0000 },  { 0x3, 0x0000 },	{ 0x4, 0x0000 },  { 0x5, 0x0008 },
	{ 0x6, 0x0000 },  { 0x7, 0x000a },  { 0x8, 0x01c0 },  { 0x9, 0x0000 },	{ 0xa, 0x00ff },  { 0xb, 0x00ff },

	{ 0x10, 0x0000 }, { 0x11, 0x007b }, { 0x12, 0x0100 }, { 0x13, 0x0032 }, { 0x14, 0x0000 }, { 0x15, 0x00c3 },
	{ 0x16, 0x00c3 }, { 0x17, 0x01c0 }, { 0x18, 0x0000 }, { 0x19, 0x0000 }, { 0x1a, 0x0000 }, { 0x1b, 0x0000 },
	{ 0x1c, 0x0000 }, { 0x1d, 0x0000 },

	{ 0x20, 0x0100 }, { 0x21, 0x0100 }, { 0x22, 0x0050 },

	{ 0x25, 0x0050 }, { 0x26, 0x0000 }, { 0x27, 0x0000 }, { 0x28, 0x0000 }, { 0x29, 0x0000 }, { 0x2a, 0x0040 },
	{ 0x2b, 0x0000 }, { 0x2c, 0x0000 }, { 0x2d, 0x0050 }, { 0x2e, 0x0050 }, { 0x2f, 0x0000 }, { 0x30, 0x0002 },
	{ 0x31, 0x0037 },

	{ 0x33, 0x0080 }, { 0x34, 0x0008 }, { 0x35, 0x0031 }, { 0x36, 0x0026 }, { 0x37, 0x00e9 },
};
...
struct wm8960_priv {
	...
	int (*set_bias_level)(struct snd_soc_component *, enum snd_soc_bias_level level);
...
/* enumerated controls */
static const char *wm8960_polarity[] = { "No Inversion", "Left Inverted", "Right Inverted", "Stereo Inversion" };
...
static int wm8960_set_deemph(struct snd_soc_component *component)
{
	...
	if (wm8960->deemph) {
		...
		for (i = 2; i < ARRAY_SIZE(deemph_settings); i++) {
			if (abs(deemph_settings[i] - wm8960->lrclk) < abs(deemph_settings[best] - wm8960->lrclk))
				best = i;
		}
	...
	return snd_soc_component_update_bits(component, WM8960_DACCTL1, 0x6, val);
}

static int wm8960_get_deemph(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int wm8960_put_deemph(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata(component);
...
static const SNDRV_CTL_TLVD_DECLARE_DB_RANGE(micboost_tlv, 0, 1, TLV_DB_SCALE_ITEM(0, 1300, 0), 2, 3,
...
static const struct snd_soc_dapm_widget wm8960_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("Left Output Mixer", WM8960_POWER3, 3, 0, &wm8960_loutput_mixer[0],
			   ARRAY_SIZE(wm8960_loutput_mixer)),
	SND_SOC_DAPM_MIXER("Right Output Mixer", WM8960_POWER3, 2, 0, &wm8960_routput_mixer[0],
...
static const struct snd_soc_dapm_widget wm8960_dapm_widgets_out3[] = {
	SND_SOC_DAPM_MIXER("Mono Output Mixer", WM8960_POWER2, 1, 0, &wm8960_mono_out[0], ARRAY_SIZE(wm8960_mono_out)),
};
...
static int wm8960_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	dev_dbg(codec_dai->dev, "%s: format is 0x%04x\n", __func__, fmt);
...
static const int bclk_divs[] = { 10, 15, 20, 30, 40, 55, 60, 80, 110, 120, 160, 220, 240, 320, 320, 320 };
 ...
 */
static int wm8960_configure_pll(struct snd_soc_component *component, int freq_in, int *sysclk_idx, int *dac_idx,
				int *bclk_idx)
{
	struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata(component);
...
static int wm8960_configure_clocking(struct snd_soc_component *component, struct snd_pcm_hw_params *params)
{
	struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata(component);
	int freq_out, i, j, k;
	u16 iface1 = snd_soc_component_read(component, WM8960_IFACE1);
	int freq_in = wm8960->freq_in;

	if (!(iface1 & (1 << 6))) {
		dev_dbg(component->dev, "Codec is slave mode, no need to configure clock\n");
		return 0;
	}
	...
	snd_soc_component_update_bits(component, WM8960_CLOCK1, 0x1, 1);
	wm8960_set_pll(component, wm8960->freq_in, freq_out);
	...
	dev_dbg(component->dev, "mclk=%d, pll=%d, lrclk=%d, bclk=%d, i=%d, j=%d, k=%d\n", wm8960->freq_in, freq_out,
		wm8960->lrclk, wm8960->bclk, i, j, k);
	return 0;
}

static int wm8960_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params,
			    struct snd_soc_dai *dai)
{
	...
	if (!wm8960->is_stream_in_use[!tx])
		return wm8960_configure_clocking(component, params);
	else {
		dev_warn(component->dev, "%s: No clock config\n", __func__);
	}

	return 0;
}

static int wm8960_hw_free(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct snd_soc_component *component = dai->component;
	struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata(component);
	bool tx = substream->stream == SNDRV_PCM_STREAM_PLAYBACK;
...
static int wm8960_set_bias_level_out3(struct snd_soc_component *component, enum snd_soc_bias_level level)
{
	struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata(component);
	int ret;

	dev_dbg(component->dev, "%s: %d -> %d\n", __func__, snd_soc_component_get_bias_level(component), level);
	switch (level) {
	...
	case SND_SOC_BIAS_STANDBY:
		if (snd_soc_component_get_bias_level(component) == SND_SOC_BIAS_OFF) {
			...
			snd_soc_component_write(component, WM8960_APOP1,
						WM8960_POBCTRL | WM8960_SOFT_ST | WM8960_BUFDCOPEN | WM8960_BUFIOEN);
...
static int wm8960_set_bias_level_capless(struct snd_soc_component *component, enum snd_soc_bias_level level)
{
	struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata(component);
	int reg, ret;
	...
	case SND_SOC_BIAS_PREPARE:
		...
		case SND_SOC_BIAS_STANDBY:
			...
			snd_soc_component_update_bits(component, WM8960_APOP1,
						      WM8960_POBCTRL | WM8960_SOFT_ST | WM8960_BUFDCOPEN,
						      WM8960_POBCTRL | WM8960_SOFT_ST | WM8960_BUFDCOPEN);
			...
			snd_soc_component_update_bits(component, WM8960_POWER2,
						      WM8960_PWR2_LOUT1 | WM8960_PWR2_ROUT1 | WM8960_PWR2_OUT3, reg);
...
static int pll_factors(unsigned int source, unsigned int target, struct _pll_div *pll_div)
{
...
static int wm8960_set_dai_pll(struct snd_soc_dai *codec_dai, int pll_id, int source, unsigned int freq_in,
...
static int wm8960_set_dai_clkdiv(struct snd_soc_dai *codec_dai, int div_id, int div)
{
	struct snd_soc_component *component = codec_dai->component;
	u16 reg;
...
static int wm8960_set_bias_level(struct snd_soc_component *component, enum snd_soc_bias_level level)
{
	struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata(component);
...
static void wm8960_set_pdata_from_of(struct i2c_client *i2c, struct wm8960_data *pdata)
{
...
static int wm8960_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	if (wm8960->pdata.mic_bias_low) {
		...
		if (ret != 0) {
			dev_err(&i2c->dev, "Failed to set MIC Bias low: %d\n", ret);
			return ret;
		}
	}
	...
	ret = devm_snd_soc_register_component(&i2c->dev, &soc_component_dev_wm8960, &wm8960_dai, 1);
...
MODULE_DESCRIPTION("ASoC WM8960 driver (corrected for PLL)");

...
static int imx_wm8960_late_probe(struct snd_soc_card *card)
{
	...
	if (!rtd) {
		dev_err(card->dev, "%s: No rtd available\n", __func__);
		return -EINVAL;
	}

	//snd_soc_dai_set_clkdiv(rtd->codec_dai, WM8960_SYSCLKDIV, wm8960_clocksetup[0].sys_clk_div);
	ret = snd_soc_dai_set_pll(rtd->codec_dai, WM8960_SYSCLK_AUTO, WM8960_PLL1, data->clk_frequency, 0);
	ret = snd_soc_dai_set_sysclk(rtd->codec_dai, WM8960_SYSCLK_PLL, data->clk_frequency, SND_SOC_CLOCK_IN);
	if (ret) {
		dev_err(card->dev, "%s: Failed (%d) to set codec clk\n", __func__, ret);
		return ret;
	}
	/* Use 0.9 factor on MIC BIAS voltage */
	snd_soc_update_bits(rtd->codec, WM8960_ADDCTL4, 0x1, 0x0);
	snd_soc_dai_set_clkdiv(rtd->codec_dai, WM8960_DCLKDIV, 7);
	if (data->use_mono_mixer)
		snd_soc_update_bits(rtd->codec, WM8960_ADDCTL1, 0x10, 0x10);
	return 0;
}
...
static int imx_wm8960_probe(struct platform_device *pdev)
{
	...
	struct device_node *ssi_np, *codec_np=0;
...
audmux_bypass:
	...
	data->use_mono_mixer = of_property_read_bool(np, "use-mono-mixer");
	data->dai.name = "HiFi";
	data->dai.stream_name = "HiFi";
	data->dai.codec_dai_name = "wm8960-lm";
